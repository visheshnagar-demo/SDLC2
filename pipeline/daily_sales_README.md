...
- **Target System**: `destination_system` (bigquery)
  - Destination Table: `analytics.daily_sales`
  - Write Mode: `append`
  - Merge Keys: `[]`
...
The container boots, runs `python -m pipeline.run_daily_sales`, loads data into the
...
## Running Locally
```bash
python -m pipeline.run_daily_sales --date $(date +%Y-%m-%d)
```

## Re-running the Pipeline
```bash
gcloud run jobs execute daily_sales --region us-central1
```
